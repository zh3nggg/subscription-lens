...
function setCompact(value){if(!win||compactMode===value)return;if(value){normalBounds=win.getBounds();win.setMinimumSize(360,400);win.setSize(420,560);}else{win.setMinimumSize(820,640);if(normalBounds)win.setBounds(normalBounds);if(pinned){pinned=false;win.setAlwaysOnTop(false);}}compactMode=value;changed();}
...
function register(){
  handler('chooseMonitor',async kind=>{if(!['cc-switch','claude','gemini','usage-jsonl'].includes(kind))throw Error('不支持的来源');const folder=['claude','gemini'].includes(kind);const result=await dialog.showOpenDialog(win,{title:t('添加监控来源'),properties:[folder?'openDirectory':'openFile'],...(folder?{}:{filters:[{name:kind==='cc-switch'?'SQLite':'JSONL',extensions:kind==='cc-switch'?['db','sqlite','sqlite3']:['jsonl']}]})});if(result.canceled)return null;const added=await service.monitor.add(kind,result.filePaths[0]);changed();return added;});
