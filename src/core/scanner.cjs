...
class Scanner{
  ...
  async scan(roots,onProgress=()=>{},deviceId=null){
    ...
    try{
      ...
      for(const file of files){try{added+=await this.read(file,deviceId);}catch{this.status.errors++;}this.status.processed++;if(this.status.processed%5===0)onProgress({...this.status});}
  ...
  async read(file,deviceId=null){
    ...
    try{const chunk=Buffer.alloc(256*1024);while(position<stat.size){const {bytesRead}=await handle.read(chunk,0,Math.min(chunk.length,stat.size-position),position);if(!bytesRead)break;position+=bytesRead;pending=Buffer.concat([pending,chunk.subarray(0,bytesRead)]);
      let at;while((at=pending.indexOf(10))>=0){const line=pending.subarray(0,at);pending=pending.subarray(at+1);committed=position-pending.length;
        ...
        try{const r=JSON.parse(line.toString('utf8'));const result=parseRecord(r,cp.state);if(result.event){if(deviceId)result.event.device=deviceId;events.push(result.event);}if(result.quota)quota=mergeQuota(quota,result.quota);if(result.warning)cp.errors++;}catch{cp.errors++;}
      }
