...
class Service extends EventEmitter{
  ...
  query(filters={}){
    ...
    // Trends need a historical baseline even when the overview period is short.
    // Keep the visible summary scoped to the selected period, while exposing a
    // filtered all-history series for the trend explorer.
    const trendTo=new Date(now.getTime()+1).toISOString();
    const trendRaw=this.store.events('2000-01-01T00:00:00.000Z',trendTo,this.device.id);
    const trendAggregate=aggregate(this.select(trendRaw,filters,sessionTitles),this.catalog);
