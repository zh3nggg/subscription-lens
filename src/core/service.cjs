...
class Service extends EventEmitter{
  ...
  select(events,filters){const query=String(filters.query||'').slice(0,200).toLowerCase();return events.filter(e=>(!filters.model||e.model===filters.model)&&(!filters.project||e.project===filters.project)&&(!filters.session||e.session===filters.session)&&(!filters.unpriced||price(e,this.catalog).amount===null)&&(!query||(e.project+' '+e.model+' '+e.session).toLowerCase().includes(query)));}
  query(filters={}){
