...
// A preview already provides the rollback path. Keep an older stable only while
// there is no preview, so the local retention policy remains exactly two builds.
if(channel==='stable')next.fallback=next.preview?null:(before.stable?.path!==folder?before.stable:null);
...
next.policy='Keep exactly the latest verified stable and preview; retain a fallback only while no preview exists.';next.updatedAt=new Date().toISOString();next.note=next.stable?(next.fallback?'Stable and fallback slots retained.':'Stable and preview slots retained.'):'No formally stable release exists; the previous preview remains the rollback copy.';
