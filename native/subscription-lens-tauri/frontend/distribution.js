...
overview=function(){const html=overviewBeforeDistribution();if(state.multi||(!state.data.settings.roots.length&&!state.data.stats.records))return html;const d=state.data;
 const deviceView=state.deviceView==='multi';
 const sourceModels=deviceView?(d.deviceCloud?.models||[]).map(g=>{const provider=distributionProviderForModel(g.model),unpriced=g.usd===null?Number(g.events||0):0;return {key:`${provider}:${g.model}`,provider,model:g.model,tokens:Number(g.tokens||0),events:Number(g.events||0),unpriced,usd:Number(g.usd||0),pricedTokens:g.usd===null?0:Number(g.tokens||0)};}):(d.modelProviders||[]);
 const modelGroups=sourceModels.map(g=>({key:g.key,provider:g.provider,model:g.model,tokens:g.tokens,requests:g.events,unpriced:g.unpriced,estimated:g.usd,reported:0,pricedTokens:g.pricedTokens}));
...
function distributionProviderForModel(model){const value=String(model||'').trim().toLowerCase(),families=[[/^codex-auto-review(?:[./:_-]|$)/,'Codex Auto Review'],[/^deepseek(?:[./:_-]|$)/,'DeepSeek'],[/^(?:qwen|qwq)(?:[./:_-]|$)/,'Alibaba Cloud'],[/^(?:kimi|moonshot)(?:[./:_-]|$)/,'Moonshot AI'],[/^(?:glm|zhipu|zai)(?:[./:_-]|$)/,'Zhipu AI'],[/^minimax(?:[./:_-]|$)/,'MiniMax'],[/^claude(?:[./:_-]|$)/,'Anthropic'],[/^gemini(?:[./:_-]|$)/,'Google'],[/^(?:gpt|o[13456])(?:[./:_-]|$)/,'OpenAI']];return families.find(([pattern])=>pattern.test(value))?.[1]||'Unknown';}
